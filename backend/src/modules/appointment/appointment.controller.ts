...
export const bookAppointment = asyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!doctor.availableDays.includes(dayName)) {
      return next(
        new ApiError(`عذراً، الطبيب غير متاح في هذا اليوم (${dayName})`, 400),
      );
    }
...
export const cancelAppointment = asyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (req.user.role === "patient") {
      ...
      const timeParts = (appointment.slotTime || "")
        .replace(/ (AM|PM)/, "")
        .split(":");
      ...
      if ((appointmentTime.getTime() - now.getTime()) / (1000 * 60 * 60) < 5) {
        return next(
          new ApiError(
            "عذراً، لا يمكن إلغاء الموعد قبل أقل من 5 ساعات من موعد الكشف",
            400,
          ),
        );
      }
...
export const getAvailableSlots = asyncHandler(
  async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    ...
    while (curr < endDateTime) {
      const slotTimeStr = curr.toLocaleTimeString("en-US", {
        hour: "2-digit",
        minute: "2-digit",
        hour12: true,
      });
